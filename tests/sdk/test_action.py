...
from kusanagi.sdk.action import Action
from kusanagi.sdk.action import NoFileServerError
from kusanagi.sdk.action import parse_params
from kusanagi.sdk.action import ReturnTypeError
from kusanagi.sdk.action import UndefinedReturnValueError
from kusanagi.sdk.file import File
from kusanagi.sdk.file import file_to_payload
...
def test_sdk_action(read_json, registry):
...
def test_sdk_action_params(read_json, registry):
    transport = Payload(read_json('transport.json'))
    params = [
        {PARAM['name']: 'foo', PARAM['value']: 1, PARAM['type']: TYPE_INTEGER},
        {PARAM['name']: 'bar', PARAM['value']: 2, PARAM['type']: TYPE_INTEGER},
...
def test_sdk_action_files(read_json, registry):
    transport = Payload(read_json('transport.json'))
    service_name = 'users'
    ...
    # Check file creation
    file = action.new_file('foo', path='/tmp/file.txt')
    assert isinstance(file, File)
    assert file.get_name() == 'foo'


def test_sdk_action_download(read_json, registry):
    service_name = 'dummy'
    service_version = '1.0'
    transport = Payload(read_json('transport.json'))
    action = Action(**{
...
def test_sdk_action_data(read_json, registry):
    transport = Payload(read_json('transport.json'))
...
def test_sdk_action_relate(read_json, registry):
    transport = Payload(read_json('transport.json'))
    address = transport.get('meta/gateway')[1]
    service_name = 'foo'
    pk = '1'
...
def test_sdk_action_links(read_json, registry):
    transport = Payload(read_json('transport.json'))
    address = transport.get('meta/gateway')[1]
    service_name = 'foo'
    link_name = 'self'
...
def test_sdk_action_transactions(read_json, registry):
    transport = Payload(read_json('transport.json'))
    service_name = 'foo'
    service_version = '1.0'
    service_action = 'foo'
...
def test_sdk_action_return_value(read_json, registry):
    service_name = 'foo'
    service_version = '1.0'
    transport = Payload(read_json('transport.json'))
...
def test_sdk_action_defer_call(read_json, registry):
    service_name = 'foo'
    service_version = '1.0'
    ...
    files = [action.new_file('download', '/tmp/file.txt')]
    ...
    assert tr_files[0] == {
        ...
        FIELD_MAPPINGS['filename']: 'file.txt',
        FIELD_MAPPINGS['size']: 0,
        FIELD_MAPPINGS['mime']: 'text/plain',
        FIELD_MAPPINGS['path']: 'file:///tmp/file.txt',
        }
...
def test_sdk_action_call_remote(read_json, registry):
    service_name = 'foo'
    service_version = '1.0'
    ...
    kwargs['files'] = [action.new_file('download', '/tmp/file.txt')]
    assert action.remote_call(**kwargs) == action
    tr_files = transport.get(files_path, delimiter='|')
    assert isinstance(tr_files, list)
    assert len(tr_files) == 1
...
def test_sdk_action_errors(read_json, registry):
    transport = Payload(read_json('transport.json'))
    address = transport.get('meta/gateway')[1]
    service_name = 'foo'
    service_version = '1.0'
